"""
MVP 17: Age-Weighted Replacement Elites

Track individual age (generations since insertion). When archive is full,
replacement considers both fitness AND age: old individuals are easier to
replace. Prevents archive staleness without explicit restart mechanisms.

Replacement score = fitness - age_penalty * age.
New individual replaces the lowest-scoring resident.
"""
...
class AgeWeightedElites(VariableConstraintGA):
    ...
    def set_up(self):
        ...
        # bins store (fitness, individual, age)
    ...
    def _replacement_score(self, fit, age):
        return fit - self.age_penalty * age
    ...
    def _place(self, ind, infeasible_pop):
        if self._const_ok(ind):
            ...
            if len(self.bins[b]) < self.inds_per_bin:
                self.bins[b].append((fit, ind, 0))
                self.bins[b].sort(key=lambda x: x[0], reverse=True)
                return True
            else:
                # Find worst by replacement score (fitness - age_penalty * age)
                worst_idx = min(range(len(self.bins[b])),
                                key=lambda i: self._replacement_score(
                                    self.bins[b][i][0], self.bins[b][i][2]))
                worst_score = self._replacement_score(
                    self.bins[b][worst_idx][0], self.bins[b][worst_idx][2])
                new_score = self._replacement_score(fit, 0)
                if new_score >= worst_score:
                    self.bins[b][worst_idx] = (fit, ind, 0)
                    self.bins[b].sort(key=lambda x: x[0], reverse=True)
                    return True
        else:
    ...
    def _select(self):
        ...
        if total_f > 0 and (total_i == 0 or
            ...
            # Select by fitness (ignore age for selection)
            pool = [(f, ind) for f, ind, age in self.bins[b]]
    ...
    def run_one_generation(self, cons_changed):
        ...
        for i in range(len(self.bins)):
            self.bins[i] = [(f, ind, age + 1) for f, ind, age in self.bins[i]]
        ...
        # Return in standard format (fitness, individual) for bins with valid individuals
        return [[(f, i) for f, i, age in bl if self._all_ok(i)] for bl in self.bins]


ALGORITHM_CLASS = AgeWeightedElites
ALGORITHM_NAME = "Age-Weighted"
ALGORITHM_KWARGS = {}
